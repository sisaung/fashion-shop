<section class="mt-8 px-5">
    <div class="flex flex-col md:flex-row items-center justify-between mt-6 gap-4">
        ...
        <div class="text-sm text-gray-700">
            Showing <span class="font-semibold">{{ $paginator->firstItem() ?? 0 }}</span>
            to <span class="font-semibold">{{ $paginator->lastItem() ?? 0 }}</span>
            of <span class="font-semibold">{{ $paginator->total() ?? 0 }}</span> entries
        </div>
            ...
            <div class="flex items-center gap-x-3">
                <div>
                    ...
                    <select wire:model="perPage"
                        class="limit appearance-none  focus:ring-pearl-bush-500 focus:ring-2 border border-gray-300 w-20 px-3 py-0.5 rounded pe-2">

                        @foreach ([5, 10, 25, 30, 50] as $limit)
                ...
                <div>
                    <span class="text-sm text-gray-700 select-none">
                        Page <b>{{ $paginator->currentPage() ?? 0 }}</b> of <b>{{ $paginator->lastPage() ?? 0 }}</b>
                    </span>
                </div>
            </div>
            <!-- Custom Pagination -->
